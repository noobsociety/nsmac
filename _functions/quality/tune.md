...
Run a named specialist evaluation command on a target, then audit cross-cutting **Criteria** and apply rubric learning via [quality-learning](../../_core/quality-learning.md).
...
3. **Route — interface:** When `<target>` is `interface`, resolve `<image>` and `<project>` from remaining tokens and attachments using the same order as **`/quality assess interface`** **Parameters** in [assess-interface](assess-interface.md). If either is missing after resolution, **ABORT** naming the missing argument.
4. Read this file and [quality-learning](../../_core/quality-learning.md). If either is not directly readable, **ABORT** per **`context-gate.md`**.
...
9. Ask once whether to append findings to [show-notes.md](show-notes.md). On confirmation: append under `## QA audit — YYYY-MM-DD`; counter-suffix if the same date heading exists. Never replace; append only.
10. For universal gaps, present proposed criteria and run [quality-learning](../../_core/quality-learning.md) **Protocol** steps 2–6. Do not edit any playbook until the gate completes and the user confirms.
...
- **Learning:** At most 2 accepted adaptation candidates applied per run; remaining deferred to next run per [quality-learning](../../_core/quality-learning.md) batch cap.
- **QA notes:** [show-notes.md](show-notes.md) is the durable append-only log for user-approved QA learning notes. Do not create a second notes file. If the file is missing when notes are approved, **ABORT** and restore the repository contract before appending.
