...
3. Load [git-convention](../../_core/git-convention.md) before generating branch names, commit subjects, PR fields, or grounded plans. If not readable, **ABORT** per **`context-gate.md`**.
4. Load [markdown-workflow](../../_core/markdown-workflow.md) when `<goal>` names devblog paths (`**/devblog/**/*.md`) or lean contracts under `doc/playbook/`. If required and not readable, **ABORT**.
...
- **Conventions:** Types, scopes, branch pattern `<type>-<scope>/<n>-<issue-title-kebab-case>`, PR keywords, and release title `chore(release): merge dev to main` follow [git-convention](../../_core/git-convention.md).
...
- **Create — Phase 2 (implement issue):** Branch and **Subject** per [git-convention](../../_core/git-convention.md). Emit **`### Phase 2 — Implement issue`**, then **Field | Value**:
...
| PR body  | `<keyword> #<n>` — keyword from **PR keyword mapping** in [git-convention](../../_core/git-convention.md) (`feat` → `Closes`, `fix` → `Fixes`, all others → `Resolves`) |
