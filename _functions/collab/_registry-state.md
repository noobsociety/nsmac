...
1. Read this document when auditing or changing `tools/collab/registry_state.py` behavior.
2. Do not mutate registry or transcript state from this documentation-only reference.

## Notes

Spec for `tools/collab/registry_state.py` — state-root and project-identity resolution. Extracted from `registry.py` in commit `a55380c`. This module handles path resolution only; it does not read or write registry or transcript data.

### Public entry

`resolve_default_registry_path(command: str | None) -> tuple[Path, bool]`

Called by `tools/collab/registry.py` before any registry read or write. Returns the resolved path to `registry.json` (at `$STATE_HOME/<projectId>/registry.json`) and a boolean indicating a default-path resolution. The `projectId` is read from the `.collab.json` marker found by walking up from `cwd`.
...
**Module-to-subcommand map row:** `participant-role-files` in `_functions/collab/_helper-output.md` covers the registry-loading commands that invoke this module's validation path.
