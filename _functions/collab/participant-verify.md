...
5. Call `tools/collab/registry.py participant-verify-state <target> <role>` to read the live participant-verification state and acquire the per-role route lock. Capture `registryRevision`, `verificationReviewSubState`, `nextRole`, and `roleState` from the output.
