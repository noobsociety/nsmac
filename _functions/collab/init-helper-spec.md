...
Canonical derivation rules for `tools/collab/registry.py init`. Both the helper implementation and the route interface description in [`init.md`](init.md) are downstream of this spec. Do not diverge either without updating this doc first.
...
1. Read this document before changing `tools/collab/registry.py init` or [`init.md`](init.md).
...
`--force` is ineligible for this route per [`command-argument.md`](../../_core/command-argument.md) guard class `registry-integrity`. The guards against duplicate records and registry corruption must not be bypassable.
