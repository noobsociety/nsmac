...
- **Sync contract compliance:** `tools/collab/registry.py advance prev` owns registry phase, turn-order restoration, and status-table rendering. No restore-path prose-rendered write is expected; any missing mirror is a helper defect under the sync contract in [`_core/route-invariant.md`](../../_core/route-invariant.md).
