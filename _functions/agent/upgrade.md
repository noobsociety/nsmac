...
4. Read the installed `AGENTS.md` and locate the `<!-- scaffold-version: <ISO-date> -->` marker line. If absent, **ABORT**: scaffold-version marker not found in `AGENTS.md`; restore the marker manually or reinstall with `/agent install` in a fresh repo.
5. Read `~/.cursor/_templates/AGENTS.md` and locate its `<!-- scaffold-version: <ISO-date> -->` marker line.
...
9. If `AGENTS.md` is in the collected overwrite set and the installed `AGENTS.md` contains a link to `cursor/_CURSOR.md` in the Entry points section: check whether the target repo's `AGENTS.md` file satisfies both (a) the `<!-- scaffold-version: ... -->` marker is present and (b) the target repo does not contain `_templates/AGENTS.md` at any tracked path. When both conditions are met, add to the candidate patch: remove the `cursor/_CURSOR.md` entry from the Entry points list. Do not apply this rewrite unless both conditions are met. Skip this step silently when `AGENTS.md` is not in the collected set or when the link is absent.
...
11. When `--force` is supplied, compute `the candidate patch` for the collected overwrite set and render the diff from `the candidate patch`. Present each collected file with its changed-block summary. Gate the overwrite per `cursor/_core/command-argument.md`, using the repository path or name as the operand:
...
- **Confirm-before-write:** The confirmation step is mandatory and not optional. Gate contract: `cursor/_core/command-argument.md`. On refusal or absent confirmation, no file is written and the marker is untouched.
...
- **Upgrade rewrite rule for `cursor/_CURSOR.md` link:** See step 9. Both conditions must be met for the rewrite to apply; otherwise the step is a no-op.
