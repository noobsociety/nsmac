...
`dimensions` values classify each role's primary review/advisory domain, drawn from a five-value taxonomy. `platform/data/domain-taxonomy.json` is the machine source of truth; the table below is its human-readable mirror.
...
| `product` | does the capability serve a real need |
| `hygiene` | dead code, drift, present-state, residue |

`dimensions` is required (non-empty) for every role except the moderator; the moderator role must not declare it — it stays coverage-only, consistent with its scope/sequencing/framing/pacing/integrity concerns. `hygiene` is part of the taxonomy but is not currently assigned as any role's `dimensions` value by design: it is cross-cutting, any role may raise it, and no command should assert per-role `hygiene` coverage the way it might for the other four domains.
