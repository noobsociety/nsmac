...
**Retired roles.** The roster above is generated from every `*.json` under `commands/collab/reference/roles/`, which includes retired stubs. `dp` (Deterministic Projector) is **retired and non-joinable** (`"joinable": false`); `dp` is not a usable role and must not be assigned to new collabs. `dp` is retained only as a backed tombstone for closed collab a13dba4c, whose participant roster references it — see [`dp.json`](../../commands/collab/reference/roles/dp.json) and the carve-out in [`invariants.md`](../../commands/collab/reference/invariants.md).
