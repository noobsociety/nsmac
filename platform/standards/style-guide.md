...
Examples of owned internal JSON: `commands/collab/reference/roles/*.json` (`key`, `displayName`, `concerns`), `$HOME/.collabs/<projectId>/registry.json` (`activeCollabId`, `turnOrder`).
...
Install layout and runtime symlink behavior are out of scope for this `platform/standards/` canon.
