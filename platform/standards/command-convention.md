...
**Stability guarantee:** The porcelain/plumbing split is intentional and stable. Adding a new multi-word route must follow the same mapping rule; a route that deviates — using hyphens in its dispatch form or spaces on disk — is a conformance failure detectable by the catalog sync harness.
