...
**Opening sentence:** The first sentence is a single declarative statement that opens with the action verb.
...
- **Brief description:** ≤120 characters on one line, or two short sentences; state what the manual covers and when to use it in place of automation — not what the project is or how to use it; no "this document" filler.
...
- **Status:** `> Last updated: YYYY-MM-DD` then one or two sentences on what is traced to current scripts and which optional branches may drift; never guess the date — prefer `date +%F` in the workspace shell, otherwise use a calendar date the user states for this edit, or ask once for permission to run `date +%F` or for the date.
