...
make_repo() {
  local dir="$1"
  mkdir -p "$dir/platform/data" "$dir/commands/demo/route"
  cat >"$dir/platform/data/source-ledger.md" <<'MD'
# Migration ledger
...
| `_mdc/auto/example.mdc` | policy | `platform/standards/example.md` | read | audit | no refs |
...
clean="$TMPDIR/clean"
make_repo "$clean"
"$ROOT/platform/tooling/check-source-ledger.py" --check --root "$clean" >"$TMPDIR/clean.out"
...
set +e
"$ROOT/platform/tooling/check-source-ledger.py" --check --root "$legacy" >"$TMPDIR/legacy.out" 2>&1
status=$?
set -e
...
blank="$TMPDIR/blank"
make_repo "$blank"
perl -0pi -e 's{\| `_mdc/auto/example\.mdc` \| policy \| `platform/standards/example\.md` \| read \| audit \| no refs \|}{| `_mdc/auto/example.mdc` | policy |  | read | audit | no refs |}' "$blank/platform/data/source-ledger.md"
set +e
"$ROOT/platform/tooling/check-source-ledger.py" --check --root "$blank" >"$TMPDIR/blank.out" 2>&1
status=$?
set -e
...
set +e
"$ROOT/platform/tooling/check-source-ledger.py" --check --root "$undeclared" >"$TMPDIR/undeclared.out" 2>&1
status=$?
set -e
