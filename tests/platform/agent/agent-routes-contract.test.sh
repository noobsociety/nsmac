...
grep -Fq 'a target path of `~/nsmac` is permitted' commands/agent/install/index.md || fail "install.md lacks valid ~/nsmac target policy"
grep -Fq 'a target path of `~/nsmac` is permitted' commands/agent/upgrade/index.md || fail "upgrade.md lacks valid ~/nsmac target policy"
grep -Fq 'A checkout developed in place at `~/nsmac` is a valid target repository root' commands/agent/install/index.md || fail "install.md lacks valid ~/nsmac target policy"
grep -Fq 'A checkout developed in place at `~/nsmac` is a valid target repository root' commands/agent/upgrade/index.md || fail "upgrade.md lacks valid ~/nsmac target policy"
