...
clean="$TMPDIR/clean"
make_fixture "$clean"
"$ROOT/commands/collab/engine/audit-reviewer-conclusion.sh" --root "$clean" >"$TMPDIR/clean.out"
...
set +e
"$ROOT/commands/collab/engine/audit-reviewer-conclusion.sh" --root "$missing_audit_gate" >"$TMPDIR/missing-audit-gate.out" 2>&1
status=$?
set -e
if [[ "$status" -eq 0 ]]; then
...
perl -0pi -e 's/validate_reviewer_conclusion_gates\(content, phase, role, current_entry\)/validate_reviewer_conclusion_gate_disabled(content, phase, role, current_entry)/' \
  "$missing_conclusion_call/commands/collab/engine/registry.py"
set +e
