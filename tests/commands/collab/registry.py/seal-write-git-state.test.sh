...
COMMITTED_DELETION_PATH="$(
  while IFS= read -r path; do
    [[ -n "$path" ]] || continue
    deleting_commit="$(git -C "$ROOT" log --diff-filter=D --format=%H -- "$path")"
    if [[ ! -e "$ROOT/$path" && -n "$deleting_commit" ]]; then
      printf '%s\n' "$path"
      break
    fi
...
seal_without_execution() {
  ...
  "$ROOT/commands/collab/engine/registry.py" seal-write "$target" pa --observed-revision "$revision" --caller-role pa "$@"
}
...
init_completion_target "Seal Write Committed Git State" "seal-write-committed-git-state"
COMMITTED_TARGET="$RUN_DATE-seal-write-committed-git-state"
...
init_completion_target "Seal Write Committed Deletion Git State" "seal-write-committed-deletion-git-state"
COMMITTED_DELETION_TARGET="$RUN_DATE-seal-write-committed-deletion-git-state"
...
init_completion_target "Seal Write Staged Git State" "seal-write-staged-git-state"
STAGED_TARGET="$RUN_DATE-seal-write-staged-git-state"
...
if [[ "$staged_status" -eq 0 || "$staged_output" != *"SEAL-GIT-STATE: implementation not in git; unstaged or uncommitted touchedPath(s) in "*"[\"$STAGED_PATH\"]"* ]]; then
  printf 'FAIL: seal-write did not reject a staged touchedPath\n%s\n' "$staged_output" >&2
  exit 1
fi

init_completion_target "Seal Write Dirty Staged Git State" "seal-write-dirty-staged-git-state"
DIRTY_STAGED_TARGET="$RUN_DATE-seal-write-dirty-staged-git-state"
...
if [[ "$dirty_status" -eq 0 || "$dirty_output" != *"SEAL-GIT-STATE: implementation not in git; unstaged or uncommitted touchedPath(s) in "*"[\"$DIRTY_STAGED_PATH\"]"* ]]; then
  printf 'FAIL: seal-write did not reject a touchedPath with unstaged changes after staging\n%s\n' "$dirty_output" >&2
  exit 1
fi

init_completion_target "Seal Write Unstaged Git State" "seal-write-unstaged-git-state"
UNSTAGED_TARGET="$RUN_DATE-seal-write-unstaged-git-state"
...
if [[ "$status" -eq 0 || "$output" != *"SEAL-GIT-STATE: implementation not in git; unstaged or uncommitted touchedPath(s) in "*"[\"$UNSTAGED_PATH\"]"* ]]; then
  printf 'FAIL: seal-write did not reject a working-tree-only touchedPath\n%s\n' "$output" >&2
  exit 1
fi
...
init_completion_target "Seal Write Work Repo Committed" "seal-write-work-repo-committed"
WORK_REPO_TARGET="$RUN_DATE-seal-write-work-repo-committed"
...
init_completion_target "Seal Write Work Repo Uncommitted" "seal-write-work-repo-uncommitted"
WORK_REPO_REJECT="$RUN_DATE-seal-write-work-repo-uncommitted"
...
if [[ "$work_repo_status" -eq 0 || "$work_repo_output" != *"SEAL-GIT-STATE: implementation not in git; unstaged or uncommitted touchedPath(s) in "*"[\"loose-file\"]"* ]]; then
  printf 'FAIL: seal-write did not gate against the declared workRepo\n%s\n' "$work_repo_output" >&2
  exit 1
fi
...
init_completion_target "Seal Write Work Repo Relative Rejected" "seal-write-work-repo-relative-rejected"
RELATIVE_WORK_REPO_TARGET="$RUN_DATE-seal-write-work-repo-relative-rejected"
set +e
...
printf 'OK: seal-write git-state gate accepts committed/deleted paths and rejects staged, unstaged, or working-tree-only paths\n'
printf 'OK: seal-write git-state gate resolves declared workRepo for cross-repo collabs\n'
