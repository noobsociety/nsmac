...
set +e
reseal_output="$("$ROOT/commands/collab/engine/registry.py" seal-render "$TARGET" pa --observed-revision "$REVISION" --caller-role pa 2>&1)"
reseal_status=$?
set -e
...
REVISION="$(assessment_revision "$TARGET")"
set +e
success_output="$("$ROOT/commands/collab/engine/registry.py" seal-render "$TARGET" pa \
