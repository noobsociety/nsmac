...
python3 - "$ROOT/commands/collab/engine/registry.py" "$ROOT/commands/collab/engine/registry_constants.py" "$ROOT/commands/collab/reference/verification.md" <<'PY'
import re
import sys
...
"$ROOT/commands/collab/engine/registry.py" init --agent-id codex --reviewer pa "Verification Cap Default" >/dev/null
...
participant_state="$("$ROOT/commands/collab/engine/registry.py" participant-verify-state "$TARGET" pe)"
