...
"$ROOT/commands/collab/engine/registry.py" init --agent-id codex "Version Field Rejection" >/dev/null
...
PY
expect_failure "registry root" "root contains disallowed version field" "$ROOT/commands/collab/engine/registry.py" validate
python3 - "$REGISTRY" "$FIELD" <<'PY'
import json
import sys
from pathlib import Path
...
path.write_text(json.dumps(data, indent=2) + '\n')
PY
expect_failure "handoff root" "handoff contains disallowed version field" "$ROOT/commands/collab/engine/registry.py" validate
...
path.write_text(json.dumps(data, indent=2) + '\n')
PY
expect_failure "handoff role" "handoff state contains disallowed version field" "$ROOT/commands/collab/engine/registry.py" validate
...
path.write_text(json.dumps(data, indent=2) + '\n')
PY
expect_failure "verification seal" "verificationSeal contains disallowed version field" "$ROOT/commands/collab/engine/registry.py" validate
...
PY
expect_failure "project identity" "project identity contains disallowed version field" "$ROOT/commands/collab/engine/registry.py" registry-path
