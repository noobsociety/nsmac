...
"$ROOT/commands/collab/engine/registry.py" show-verdict "$TARGET" >verdict.json
python3 - <<'PY'
import json
...
set +e
missing_output="$("$ROOT/commands/collab/engine/registry.py" show-verdict "$RUN_DATE-show-verdict-output-missing" 2>&1)"
missing_status=$?
set -e
