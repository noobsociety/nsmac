...
"$ROOT/commands/collab/engine/registry.py" set "$FOLLOW_TARGET" active-phase Completion --force --caller-role mod >/dev/null
...
follow_state="$("$ROOT/commands/collab/engine/registry.py" seal-state "$FOLLOW_TARGET" pa)"
follow_revision="$(read_json_field registryRevision <<<"$follow_state")"
set +e
missing_output="$("$ROOT/commands/collab/engine/registry.py" seal-render "$FOLLOW_TARGET" pa --observed-revision "$follow_revision" --cap-exit follow-up-collab --caller-role pa 2>&1)"
missing_status=$?
set -e
...
follow_output="$("$ROOT/commands/collab/engine/registry.py" seal-render "$FOLLOW_TARGET" pa \
...
assert entry['verificationSeal']['followUp']['evidence']['committedPaths'] == ['platform/tooling/audit.sh']
PY
