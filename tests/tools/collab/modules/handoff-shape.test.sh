...
from tools.collab import handoff_shape as h
...
assert h.validate_handoff_write_scope([' tools/collab/ ', 'tools/*.py']) == ['tools/collab', 'tools/*.py']
...
assert h.handoff_state_for_role(entry, 'pe')['writeScope'] == ['tools/collab']
