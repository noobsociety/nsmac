...
if ! grep -Fq 'If Step 7 found no unchecked assigned items' "$ROOT/commands/collab/run-plan/index.md"; then
  printf 'FAIL: run-plan Step 8 does not condition prior completed execution on Step 7 having no unchecked items\n' >&2
  exit 1
fi

if ! grep -Fq 'a later successful `execution` helper write replaces the role' "$ROOT/commands/collab/run-plan/index.md"; then
