...
"$ROOT/tools/collab/registry.py" init --agent-id codex "Full Body Flow" >/dev/null
...
"$ROOT/tools/collab/registry.py" set "$TARGET" active-phase Discussion --force --caller-role mod >/dev/null
...
"$ROOT/tools/collab/registry.py" retract-speak "$TARGET" pe --reason "test withdrawal" --caller-role pe >/dev/null
...
"$ROOT/tools/collab/registry.py" init --agent-id codex "Full Body Reject Details" >/dev/null
"$ROOT/tools/collab/registry.py" join-participants "$REJECT_TARGET" pe --agent-id gpt >/dev/null
"$ROOT/tools/collab/registry.py" set "$REJECT_TARGET" turn-order pe --caller-role mod >/dev/null
reject_state="$("$ROOT/tools/collab/registry.py" speak-state "$REJECT_TARGET" pe)"
...
set +e
reject_output="$("$ROOT/tools/collab/registry.py" speak-render "$REJECT_TARGET" pe --content-file bad-excerpt.md --observed-revision "$reject_revision" --caller-role pe 2>&1)"
reject_status=$?
set -e
