...
"$ROOT/tools/collab/registry.py" execution "$TARGET" pe completed "2026-05-17T12:00:00+02:00" \
  --assigned-role pe \
...
speak_state="$("$ROOT/tools/collab/registry.py" speak-state "$TARGET" pe)"
...
set +e
blocked_output="$("$ROOT/tools/collab/registry.py" seal-render "$TARGET" pa --observed-revision "$(python3 -c 'import json,sys; print(json.load(sys.stdin)["registryRevision"])' <<<"$seal_state")" --caller-role pa 2>&1)"
blocked_status=$?
set -e
...
reviewer_speak_state="$("$ROOT/tools/collab/registry.py" speak-state "$TARGET" pa)"
