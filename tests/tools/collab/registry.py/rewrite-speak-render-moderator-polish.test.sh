...
"$ROOT/tools/collab/registry.py" init --agent-id codex "Rewrite Moderator Polish" >/dev/null
...
"$ROOT/tools/collab/registry.py" set "$TARGET" turn-order "mod pe" --caller-role mod >/dev/null

revision="$("$ROOT/tools/collab/registry.py" speak-state "$TARGET" mod \
  | python3 -c 'import json,sys; print(json.load(sys.stdin)["registryRevision"])')"

printf 'initial moderator text\n' >initial.md
"$ROOT/tools/collab/registry.py" speak-render "$TARGET" mod \
...
printf '+ speek now\n' >rewrite-polished.md
"$ROOT/tools/collab/registry.py" rewrite-speak-render "$TARGET" mod \
...
spec = importlib.util.spec_from_file_location("registry", root / "tools/collab/registry.py")
