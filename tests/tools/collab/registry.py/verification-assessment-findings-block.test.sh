...
"$ROOT/tools/collab/registry.py" reopen "$TARGET" action-plan --caller-role mod >/dev/null
...
"$ROOT/tools/collab/registry.py" set "$SUCCESS_TARGET" active-phase Completion --force --caller-role mod >/dev/null
