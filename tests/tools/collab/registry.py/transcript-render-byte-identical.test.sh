...
from tools.collab.registry import (
    append_phase_block,
...
transcript, changed = render_managed_header_text(transcript, entry, Path('core/collab/roles'))
if not changed:
...
transcript = append_reviewer_findings_block(
    ...
    'pa',
    {
        ...
        'evidence': {
            'registryRevision': 7,
            'committedPaths': ['tools/collab/registry.py'],
...
commandPacket:
  ...
  AFFECTED: committedPaths=["tools/collab/registry.py"]; executionEntryIds=["exec-1"]; transcriptIds=["audit-tw-1"]
  RETURN: Action Plan
