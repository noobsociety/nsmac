...
Deterministic QA for `~/.cursor/core/framework/*.md` canon documents.

## Procedure

1. Load every `*.md` under `~/.cursor/core/framework/`.
2. Validate the roster is exact.
3. Validate each file has one H1 and is <= 250 lines.
4. Validate links in `core/framework/` stay self-contained (sibling `core/framework/*.md` links only).
5. Validate no `core` file depends on `commands/`, core policy, core policy, `tests/specs/`, or host runtime paths.
