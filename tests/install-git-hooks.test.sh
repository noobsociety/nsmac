...
CURSOR_GIT_HOOKS_DIR="$hooks_dir" "$ROOT/tools/cursor/install-git-hooks.sh" >"$TMPDIR/install.out"
...
set +e
CURSOR_GIT_HOOKS_DIR="$hooks_dir" "$ROOT/tools/cursor/install-git-hooks.sh" >"$TMPDIR/reinstall.out" 2>&1
status=$?
set -e
