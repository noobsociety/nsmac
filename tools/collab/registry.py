...
from tools.collab.transcript_render import (
    append_phase_block,
    insert_toc_entry,
...
#   tools.collab.registry_io         — registry persistence, lock, resolve; validator injection (no phase decisions)
#   tools.collab.participants        — participant roster, reviewer wiring, turn-order helpers (no phase mutation)
#   tools.collab.phase_lifecycle     — phase sequencing and lifecycle notices (no registry mutation/rendering)
#   tools.collab.execution           — execution checks, run-plan support, write-scope enforcement (no seal)
#   tools.collab.transcript_render   — managed rendering: all <details> blocks (rendered_collapsible_block), header, TOC, contributions, revision history, retracted content (no registry mutation)
...
#   4. seal/verification engine          — pending (#58: seal_verification.py, highest integrity cost, last)
...
# cleanly from tools.collab.registry), not just the path. Invariant #17's
...
def participant_verification_incomplete(entry: dict) -> bool:
    return participant_verification_enabled(entry) and not all_participant_verification_completed(entry)
...
def participant_verify_state(path: Path, target: str, role: str, resume: bool = False) -> int:
    ...
    if resume:
        result['resume'] = f'tools/collab/registry.py participant-verify-state --resume {entry["id"]} {role}'
    print(json.dumps(result, sort_keys=True))
    return 0
...
def participant_verify_render(
    ...
    with registry_lock(path):
        ...
        if observed_revision != live_revision:
            die(
                f'stale registry revision: observed {observed_revision}, live {live_revision}\n'
                f'RESUME: tools/collab/registry.py participant-verify-state --resume {entry["id"]} {role}'
            )
        if not has_participant(entry, role):
            die(f'role must already be a participant: {role}')
        if reviewer_backed(entry) and all_execution_completed(entry):
...
def render_registry_cli_doc() -> str:
    ...
    lines = [
        '# Registry CLI',
        '',
        '_Generated by `tools/collab/registry.py registry-cli-doc`; do not edit by hand._',
        '',
        '## Global options',
        '',
    ...
    if args.command == 'registry-cli-doc':
        ...
        if args.check:
            if not generated_path.exists() or generated_path.read_text() != rendered:
                die('generated/registry-cli.md is stale; run tools/collab/registry.py registry-cli-doc > generated/registry-cli.md')
            return 0
        print(rendered, end='')
        return 0
