...
The local `.collab.json` marker binds this checkout to a readable project id.

Runtime state is intentionally excluded from git. Source lives in this repository; generated mirrors
live under `generated/`; local runtime payloads such as command config directories, tracking payloads,
`argv.json`, `projects/`, `extensions/`, `ide_state.json`, `plugins/`, `skills/`, `plans/`,
`subagents/`, `.collab.json`, and `.DS_Store` remain ignored.
