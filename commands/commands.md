...
- **`(collab <init | join | speak | retract speak | rewrite speak | advance | restore | set | unset | list | activate | open | reopen | close | remove participant | archive | delete | write summary | rewrite summary | run plan | aggregate | export-issues | rewrite execution | participant verify | show policy | show flags | show verdict | seal verification>)`** — create, join, contribute to, retract or rewrite the last contribution, move between phases, reopen from non-success verdicts, rewrite execution records, edit or clear scoped metadata, manage active collabs, reopen/close, manage participants, soft-delete (archive) or permanently delete, write or rewrite summaries, run assigned action-plan items, refresh the moderator project transcript, record issue-terminal handoff evidence, record participant verification, read the gate policy, display the generated flag inventory or verdict metadata, or seal the `Completion.verification` sub-state after a reviewer pass; moderator-role contributions require human-authored text passed as `<message>` to `(collab speak "<message>")` after joining with the moderator role.
...
| `(collab aggregate)` | [collab/aggregate/index.md](collab/aggregate/index.md) |
...
| `(collab init "<name>" [--reviewer <role>] [--terminal <seal\|issue>] [--no-participant-verification] [--work-repo <path>] [--preview])` | [collab/init/index.md](collab/init/index.md) |
