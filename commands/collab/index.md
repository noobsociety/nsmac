...
Route moderated collaboration-record workflows through one public command router.

## Trigger

**Dispatch:** `(collab <init | join | speak | retract speak | rewrite speak | advance | restore | set | unset | list | activate | open | reopen | close | remove participant | archive | delete | write summary | rewrite summary | run plan | aggregate | export-issues | rewrite execution | participant verify | show policy | show flags | show verdict | seal verification>)` — routing-only command form; not a shell command.
