...
**Dispatch:** `(collab <init | join | speak | retract speak | rewrite speak | advance | restore | set | unset | list | activate | open | reopen | close | remove participant | archive | delete | diff | write summary | rewrite summary | run plan | summarize | export-issues | rewrite execution | participant verify | show policy | show flags | show verdict | seal verification | log | status>)` — routing-only command form; not a shell command.
