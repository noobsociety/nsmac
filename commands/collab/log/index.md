...
- **Data source:** Reads from `<state-root>/revisions/<collabId>/` — the append-only revision store introduced by collab #52 (`collab-state-observability`). If the `revisions/` directory is absent, or present but has no event matching the collab, the helper synthesizes a single `legacy-baseline` entry for that collab rather than printing nothing. This is the retroactive behavior for pre-existing collabs (those initialized before the revision store existed); see [schema-evolution.md § Compatibility readers](../../../commands/collab/reference/schema-evolution.md) for the `legacy-baseline.json` sentinel. The helper always exits `0` and prints at least one line.
...
- **Naming disambiguation:** "Registry changelog" is the correct framing for this command's output. Do not conflate it with the `<details><summary>Revision history</summary>` blocks in transcript contributions, which record per-contribution rewrites and are written by `prepend_revision_history` (`transcript_render.py:405`). These are two distinct history surfaces: one for registry state events, one for transcript content edits.
- **Read-only:** The route does not mutate registry state or transcript text.
- **Precondition:** `(collab log)` requires the append-only revision writer to be present. If the writer is absent, log output falls back to a synthesized `legacy-baseline` line for every collab rather than the real event history — see **Data source** above. The writer is a hard precondition for this route's usefulness; its implementation and `eventIndex` counter are a single deliverable.
