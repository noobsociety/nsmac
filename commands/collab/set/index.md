...
- **Sync contract compliance:** `commands/collab/engine/registry.py set` is helper-rendered: every field branch (`title`, `description`, `turn-order`, `reviewer`, `reviewer-optional-phases`, `active-phase`, `work-repo`) falls through to the same managed-header render and atomic registry-plus-transcript commit before returning, so no settable field requires a separate agent-authored transcript step. See the sync contract in [`platform/standards/route-invariants.md`](../../../platform/standards/route-invariants.md).
