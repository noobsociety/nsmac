...
- **terminal** — the workflow-model selector stored in the registry `terminal` field, set at collab init time via `--terminal <seal|issue>`. Determines how the collab record closes: `seal` (default) closes via reviewer seal verification; `issue` closes via exported issue handoff evidence without a reviewer seal. Valid values: `seal`, `issue` (enumerated in `registry_constants.py:ALLOWED_TERMINALS`). The terminal value is immutable after init. Related: [workflow models](workflow-models.md), [issue terminal](#issue-terminal), [`workflow model`](#workflow-model), [`REPOSITORY.md` §6](../../../REPOSITORY.md#6-collab-workflow-models).

- **workflow model** — the lifecycle contract that governs how a collab record closes, selected by the `--terminal` flag at init time. Two models are committed: the seal workflow model (default, `--terminal seal`, closes via reviewer seal verification) and the issue workflow model (`--terminal issue`, closes via exported issue handoff evidence). The model is immutable after init. See [`workflow-models.md`](workflow-models.md) for the full doctrine. Related: [`terminal`](#terminal), [issue terminal](#issue-terminal), [`REPOSITORY.md` §6](../../../REPOSITORY.md#6-collab-workflow-models).

- **issue terminal** — a collab configured with `--terminal issue`, using the issue workflow model. Closes via externally supplied exported issue handoff evidence recorded by `/collab export-issues`; does not enter `Completion.verification` and does not require a `verificationSeal`. Related: [workflow models](workflow-models.md), [`terminal`](#terminal), [`workflow model`](#workflow-model), [`REPOSITORY.md` §6](../../../REPOSITORY.md#6-collab-workflow-models).
