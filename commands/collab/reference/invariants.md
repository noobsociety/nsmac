...
**Reopen carry-forward caveat:** Reopen carry-forward coverage is content-validated against `HEAD`; a carried chartered path that is removed or content-drifted is dropped from the coverage aggregate, so a name-only carry cannot mask a reverted deliverable. See Invariant #21 for the full carry-forward rule and `reopenCoverage` lifecycle.
...
When `/collab reopen` is called on a collab that has completed execution entries, `reopen_collab` (`commands/collab/engine/registry.py`) snapshots the current coverage aggregate — active execution entries plus any surviving prior carry — into the collab's top-level `reopenCoverage` field before resetting the active execution state. At seal time, `valid_carried_execution_entries` (`commands/collab/engine/digests.py`) re-validates each entry in the snapshot against current `HEAD` content-digests: a carried path whose blob is absent from `HEAD` or whose digest has drifted is silently dropped. Surviving carried paths are included in both the `touchedPaths` aggregate and the content-digest set (Invariant #20), so they participate in seal integrity. A carry-forward that adds only path names to the coverage set without this content-digest re-validation would let a removed or reverted chartered deliverable appear covered at the Invariant #17 gate; the implementation rejects this form.

**`reopenCoverage` lifecycle:** Written by `reopen_collab` at `registry.py:3218` when the pre-reopen coverage snapshot is non-empty (`{ createdAt: ISO-8601, executionEntries: object[] }`). If a collab is reopened multiple times, each reopen re-snapshots `execution_coverage_entries` at that moment — which includes the current round's active execution entries and any previously valid carry — so the snapshot transitively re-includes earlier rounds' coverage (subject to the same content-validation pass). If there are no execution entries at the time of reopen, `reopenCoverage` is not written; a stale value from a prior reopen round may persist in the registry but is superseded at next reopen when coverage entries exist. `reopenCoverage` is not explicitly cleared after close; it is ignored by `valid_carried_execution_entries` when the collab has no active reopened state.

**Maintainer check:** `tests/commands/collab/registry.py/reopen-incremental-coverage.test.sh` covers four cases: (1) **preserved** — broad execution → narrow reopen → prior path still at `HEAD` → seal passes; (2) **drifted** — broad execution → narrow reopen → prior path content changed in the reopen round → seal fails; (3) **transitive** — two successive narrow reopens → first round's still-present path survives both carry hops → seal passes; (4) **removed** — broad execution → narrow reopen → prior path deleted from repo → seal fails (`CHARTERED-DELIVERABLE-MISSING`). Any change to `valid_carried_execution_entries` or `reopen_collab`'s snapshot logic must preserve all four behaviors.
