...
Canonical derivation rules for `commands/collab/engine/registry.py init`. Both the helper implementation and the route interface description in [`init.md`](../init/index.md) are downstream of this spec. Do not diverge either without updating this doc first.
...
1. Read this document before changing `commands/collab/engine/registry.py init` or [`init.md`](../init/index.md).
...
The moderator's `agentId` is captured once at init time, not at first speak. The route must pass the declared helper flag `--agent-id <agentId>`, applying the shared vocabulary and precedence in [agent-id.md](agent-id.md).
