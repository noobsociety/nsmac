...
Document the gate policy that decides when a collaboration needs a reviewer judgment pass, and list available joinable roles.

## Trigger

**Dispatch:** `(collab show policy)` — routing-only command form; not a shell command.
...
- **Honor-system / helper drift:** A route stays marked `agent-honor-system` while the underlying helper begins enforcing the same abort path. The P9 coverage gate (`platform/tooling/coverage-gate.sh`) detects both directions: missing tests for helper-enforced paths, and stale `agent-honor-system` markers — when such a clause's ABORT anchor also has a matching P9 test, the gate fails with `stale agent-honor-system marker(s)` and requires the marker's removal. Promote a path by adding its test and dropping the marker in the same change.
