...
- **Coverage carry-forward locus:** Coverage is preserved at reopen time by snapshotting current execution coverage (active entries plus any valid prior carry) into the collab's top-level `reopenCoverage` field (`{ createdAt: ISO-8601, executionEntries: object[] }`) before the active execution state is reset. At seal time, `valid_carried_execution_entries` (`commands/collab/engine/digests.py`) reads this snapshot, re-validates each entry against current `HEAD` content-digests, and merges the surviving entries into the coverage aggregate alongside the current round's active execution. Carried paths that have been removed from `HEAD` or whose content has drifted are silently dropped. A `record_execution` merge is not a valid fix locus: it wholesale-replaces the role's active execution row and breaks content-addressing (Invariant #20). See Invariant #21 in [invariants.md](../../../commands/collab/reference/invariants.md) for the `reopenCoverage` lifecycle.
