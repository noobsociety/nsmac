...
- **Seal-desync caution:** Restoring a collab that has an existing `verificationSeal` to a pre-seal event index leaves the seal in place in the live registry. The seal evidence (`executionEntries`, `touchedPaths`) may no longer match the restored entry's execution state. After a content restore on a sealed collab, run `(collab diff)` to check for drift before re-sealing.
