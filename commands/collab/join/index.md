...
3. Read `../../../core/collab/roles/<role>.json`. If unreadable, **ABORT**: role file unreadable; name the expected path.
...
9. Declare the active runtime harness identity as `--agent-id` before invoking the helper, using the precedence in [agent-id.md](../../../core/collab/agent-id.md). The value must come from the current harness or execution environment. Do not copy from role files, prior collab records, test fixtures, examples, or documentation. If the harness does not expose a usable identity, pass the literal `unknown`. This step applies to all roles, including the moderator.
10. Call `tools/collab/registry.py join-participants <target> --agent-id <agentId>`. The helper validates all role files, enforces a non-blank `agentId`, computes the next registry participants and turn order in memory, renders the full participants table from that state, then writes registry and transcript together. The first output line is `NEXT: Run /collab show policy before first speak.`; display it before the participant roster output. Abort if any role file is missing or unreadable, or if `agentId` is missing or blank, before either file is written.
...
- **Role schema:** Defined in [core/framework/agent-role.md](../../../core/framework/agent-role.md). Required fields: `key` (must match `<role>`), `displayName`, `concerns` (non-empty array). Optional field: `prohibitions` (string array; principle-level behavioral constraints, not a runtime enforcement list). For per-role join-time model recommendations, see [`agent-model.md`](../../../core/collab/agent-model.md).
...
- **Role data boundary:** Role JSON files live under `core/collab/roles/` as shared command-owned data, not route playbooks.
...
- **After joining:** The helper emits `NEXT: Run /collab show policy before first speak.` as its first output line. Run `/collab show policy` before your first `/collab speak` to read the gate rules and reviewer contract for this collab. The machine-verifiable resume signal is: `tools/collab/registry.py speak-state --resume <target> <role>`. Run this after any `/compact`, agent swap, or subagent return to re-establish collab context before writing.
- **Sync contract compliance:** Step 11's Turn order cell sync is prose-rendered; `join-participants` renders the participants table but does not own the state-table Turn order cell. This is declared under the sync contract in [`core/framework/route-invariant.md`](../../../core/framework/route-invariant.md).
- **Advisory output contract (join):** After a successful join, the helper emits advisory lines in this fixed order: `NEXT:` first (imperative routing, e.g. `Run /collab show policy before first speak.`), `EFFORT (next-speak):` second (carrying current `activePhase` · joining role · effort level · scale phrase derived from `core/collab/agent-effort.json`), `IDENTITY:` third (the at-join `agentId`, so the moderator can verify the recorded identity without reading the registry). `IDENTITY:` reflects the captured at-join identity only; it does not overwrite registry on cross-harness re-join. If a re-join supplies a different `agentId`, the helper emits `IDENTITY-WARN:` after `IDENTITY:` and keeps the registry value unchanged. No `EFFICIENCY:` line is emitted at join time; the join action does not cross a lifecycle boundary. Gate failures emit `RESUME: tools/collab/registry.py speak-state --resume <target> <role>` as a terminal advisory line so agents can re-establish state without consulting documentation. Consumers must parse by prefix label, not by line index; fixed ordering is a readability convention only.
