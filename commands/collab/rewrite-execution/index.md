...
<!-- abort: rewrite-execution-record-unreadable -->
2. Read the resolved registry and the resolved transcript path. If either is unreadable, **ABORT** (agent-honor-system): record unreadable; name the path. The `execution` and `execute-spawn` helpers tolerate a missing transcript rather than aborting, so this read-time guard is agent-enforced.
<!-- abort: rewrite-execution-record-is-closed -->
3. If the registry status is `closed` or `archived`, **ABORT**: closed collaboration records cannot be re-executed. The `execution` helper backstops this with `record is closed`.
<!-- abort: rewrite-execution-active-phase-missing -->
...
<!-- abort: rewrite-execution-phase-not-completion -->
5. If the active phase is not `Completion`, **ABORT**: `(collab rewrite execution)` is valid only when registry `activePhase` is `Completion`. The `execute-spawn` helper enforces the same gate before any subagent scope check, aborting with `execute-spawn is valid only in Completion`.
<!-- abort: rewrite-execution-role-not-registered -->
...
<!-- abort: rewrite-execution-last-already-succeeded -->
7. Read `## Completion` and locate the role's last execution-history entry. If the last entry is already marked `completed`, **ABORT** (agent-honor-system): last execution already succeeded; nothing to retry. This Completion-history precondition is read from the transcript prose and is not mirrored by an `execution` helper guard.
<!-- abort: rewrite-execution-no-prior-execution -->
8. If no execution-history entry exists for this role, **ABORT** (agent-honor-system): no prior execution record to rewrite; use `(collab run plan)` to begin execution. The absence of a prior Completion-history entry is observed in the transcript prose and is not enforced by the `execution` helper.
