...
1. Read [invariants.md](../../../core/collab/invariants.md) before executing; call the relevant helper fresh and do not trust prior reads from conversation context (Invariant #4). Resolve `<record>` from the next positional token after `activate`. If missing, **ABORT**: `<record>` is required.
