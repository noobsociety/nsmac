...
- **Release boundary:** Close is a lifecycle-state transition and notification surface only. It must not call `commands/collab/engine/registry.py tag`, `commands/collab/engine/registry.py release`, `git push`, pull-request creation, direct merge, GitHub release creation, or changelog generation. Use `(collab tag)` or `(collab release)` explicitly after the collab has reached the required release point.
