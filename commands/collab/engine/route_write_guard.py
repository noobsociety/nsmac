#!/usr/bin/env python3
"""Route write-path guard (`write-guard`): confirm a route only writes under the user-scope collab state root — the registry file, its lock, and the `records/` transcript tree — rejecting absolute paths and any target outside that set. The `execute` route is exempt (it writes through the work repo, not the state root). Self-contained: no registry state, transcript reads, or commit primitives — only path-shape validation."""
