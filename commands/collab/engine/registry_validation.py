...
from commands.collab.engine.seal_verification_logic import validate_verdict
...
def validate_root_fields(data: dict, source: str) -> None:
...
def validate_identity_metrics(data: dict, source: str) -> None:
...
def validate_project_metadata(data: dict, source: str) -> None:
...
def validate_required_collab_fields(
    entry: dict,
    source: str,
    collab_id: object,
    slug: object,
    ...
    if transcript_path != f'records/{collab_id}.md':
...
def validate_participants(participants: object, moderator_role: str, roles_dir: Path | None, source: str) -> list[str]:
    ...
    if moderator_role not in participant_role_keys:
        die(f'{source}: moderatorRole must be listed in participants')
    return participant_role_keys


def validate_turn_order(turn_order: object, participant_role_keys: list[str], source: str) -> None:
    ...
    if not set(turn_order).issubset(set(participant_role_keys)):
...
def validate_reviewer_fields(
    ...
    if reviewer_role == moderator_role:
    ...
    if mode == 'last-in-convergent-phases' and reviewer_role in turn_order:
...
def validate_execution_states(execution: object, source: str) -> None:
...
def validate_completion_state(completion: object, source: str) -> None:
...
def validate_verification_seal_state(
    verification_seal: object,
    source: str,
    collab_id: str,
    active_id: object,
) -> None:
    if verification_seal is None:
        return
...
def validate_registry_collection(
    source: str,
    ids: list[str],
    slugs: list[str],
    sequences: list[int],
...
def validate_registry(data: dict, path: Path | None = None, roles_dir: Path | None = None) -> None:
    ...
    validate_root_fields(data, source)
    validate_identity_metrics(data, source)
    validate_project_metadata(data, source)
    ...
    for entry in collabs:
        ...
        validate_required_collab_fields(
            entry,
            source,
            collab_id,
            slug,
        ...
        participant_role_keys = validate_participants(participants, moderatorRole, roles_dir, source)
        validate_turn_order(turnOrder, participant_role_keys, source)
        validate_reviewer_fields(entry, source, reviewerRole, moderatorRole, turnOrder, created_at)
        validate_execution_states(entry.get('execution', {}), source)
        ...
        validate_handoff(entry.get('handoff'), source, collab_id, active_id, participant_role_keys)
    ...
    validate_registry_collection(source, ids, slugs, sequences, active_id, collab_map)
