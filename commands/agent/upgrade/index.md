...
5. Read `~/.cursor/templates/AGENTS.md` and locate its `<!-- scaffolded-at: <ISO-date> -->` marker line.
...
11. When `--force` is supplied, compute `the candidate patch` for the collected overwrite set and render the diff from `the candidate patch`. Present each collected file with its changed-block summary. Gate the overwrite per `core/framework/command-argument.md`, using the repository path or name as the operand:
...
- **Boundary:** Reads installed scaffold files and `~/.cursor/templates/`; writes only `AGENTS.md`, `CLAUDE.md`, and (when overlap-free) `REPOSITORY.md` in the repo root. Leaves template sources, command sources, rule sources, and agent settings JSON unchanged.
...
- **Confirm-before-write:** The confirmation step is mandatory and not optional. Gate contract: `core/framework/command-argument.md`. On refusal or absent confirmation, no file is written and the marker is untouched.
