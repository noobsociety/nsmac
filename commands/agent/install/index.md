...
Install the multi-agent scaffold into the current repository from `~/.cursor/templates/`.
...
2. Verify `~/.cursor/templates/CLAUDE.md`, `~/.cursor/templates/AGENTS.md`, and `~/.cursor/templates/REPOSITORY.md` all exist. If any is missing, **ABORT** naming the missing path.
3. Parse flags immediately after the route selector and before any positional arguments per [core/framework/command-argument.md](../../../core/framework/command-argument.md). `--force` is supported only in that pre-positional slot. Unsupported or misplaced flags **ABORT** before any route mutation.
...
6. Copy `~/.cursor/templates/CLAUDE.md` to `<repo-root>/CLAUDE.md`.
7. Copy `~/.cursor/templates/AGENTS.md` to `<repo-root>/AGENTS.md`.
8. Copy `~/.cursor/templates/REPOSITORY.md` to `<repo-root>/REPOSITORY.md`. These copy steps apply `the candidate patch` without recomputation or re-read of source.
...
- **Scaffold marker:** The `<!-- scaffolded-at: ... -->` marker line is present in `~/.cursor/templates/AGENTS.md` and copied verbatim to the installed `AGENTS.md`. This marker is used by `/agent upgrade` to detect when an upgrade is needed.
