...
3. Read `core/collab/roles/<key>.json`. If unreadable, **ABORT**: role file unreadable; name the expected path.
4. Validate the role JSON against [core/framework/agent-role.md](../../../core/framework/agent-role.md): `key` must match `<key>`, and `displayName` and non-empty `concerns` must be present. If invalid, **ABORT**: invalid role JSON; name the failed field.
...
- **State helper:** `tools/narrative/state.py` owns state paths, schema validation, rerun mode, runtime guard, phase transitions, artifact ingestion, and role-derived `concernRequirements` writes. Subcommands: `audit`, `align`, `record`, `gate`.
...
- **Role contract:** [core/framework/agent-role.md](../../../core/framework/agent-role.md) — schema, key uniqueness, and canonical `--role` invocation.
...
- **Phase output writes:** After a phase emits its structured artifact, commit it to `phaseOutputs[phase]` by calling `tools/narrative/state.py record <audit|align> --role <key> --artifact <path>`. The artifact path must contain a JSON object matching the phase fields below; pass `--artifact -` to read that JSON object from stdin. `align` must read `phaseOutputs.audit` before producing output. `gate` must read both `phaseOutputs.audit` and `phaseOutputs.align` before producing output. Never rely on chat history or prose outside the state file for prior-phase handoff.
- **Artifact file format:** The `--artifact <path>` file passed to `record` must be a JSON object — not the plain-text template output. Key names match the template section headers converted to camelCase: `audit` keys are `driftThemes`, `styleViolations`, `recommendedScope`, `filesToEdit`, `auditScopeBaseline`, `coveredConcerns`; `align` keys are `aligned`, `mismatched`, `missingLocally`, `missingGlobally`, `coveredConcerns`.
...
- **Concern coverage hard gate:** Each phase artifact must include `coveredConcerns`. For `audit` and `align`, the scaffold seeds `coveredConcerns: []`; only `record` writes concern data, sourced from the agent's submitted artifact. For `gate`, the artifact self-reports `list(role["concerns"])` as it is a report-only phase. Verifying that the artifact has non-empty content for every claimed concern is agent responsibility. On a miss, emit the full artifact and name missing concern keys. For `audit` and `align`, **ABORT** after emitting the artifact; for `gate`, report `Result: fail`.
...
- **Audit surfaces:** Every narrative file under `~/.cursor/` is a valid audit surface: `core/framework/`, `commands/`, `tests/specs/`, and core policy.
- **Align surface:** Project-local rule files checked against their counterparts under the *project overlay* (the global `~/.cursor/` counterpart of each project-local rule file). Failure modes: file present locally but absent globally, or present globally but absent locally.
- **Phase 1 — Audit:** Do not edit files. Resolve and write the state file with `tools/narrative/state.py audit --role <key>`, which writes `roleBindings.audit` and `concernRequirements.audit` from the resolved role. After emitting the artifact, verify `coveredConcerns` and call `tools/narrative/state.py record audit --role <key> --artifact <path>` to commit the artifact to `phaseOutputs.audit`; on coverage miss, emit the artifact, name missing keys, and **ABORT** before handing off. Emit:
...
- **Phase 2 — Align:** Read-only phase — do not edit any files. Call `tools/narrative/state.py align --role <key>` before comparing files to write `roleBindings.align` and `concernRequirements.align` from the resolved role. Enumerate project-local rule files and compare each against `project overlay`. Read `phaseOutputs.audit` from state before producing output; if missing or malformed, **ABORT** naming `phaseOutputs.audit`. Report mismatches — do not auto-resolve. Verify `coveredConcerns` and call `tools/narrative/state.py record align --role <key> --artifact <path>` to commit the artifact to `phaseOutputs.align`; on coverage miss, emit the artifact, name missing keys, and **ABORT** before handing off. Emit:
