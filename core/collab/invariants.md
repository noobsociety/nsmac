...
**Enforcement note:** Tag-level validation is enforced by `validate_action_plan_shape()` (`tools/collab/registry.py:3161`) calling `action_plan_item_tag()` (`tools/collab/registry.py:612`): `speak-render` rejects unrecognized or missing item tags; the Action Plan close path's `validate_action_plan_executable_scope()` (`tools/collab/registry.py:3196`) rejects plans without at least one `[execute]` item.
