...
- **`caller-declined`** — the reserved `--agent-id` token for explicit identity opt-out: the harness exposes a usable identity but the caller deliberately chooses not to declare it. Distinct from `unknown`, which is reserved for the harness-inaccessible case only (the harness cannot expose any identity). The helper counts `caller-declined` joins before rejection enforcement; policy on whether to allow or reject it is collab-configuration-owned. See [`agent-id.md`](../../core/collab/agent-id.md).
