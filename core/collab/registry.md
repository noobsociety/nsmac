...
- **Per-collab entry fields:**
  ...
  | `participants` | `{ role: string, agentId: string }[]` | Ordered list of registered participants. Each entry records the role key and the joining agent's at-join `agentId` per [agent-id.md](../../core/collab/agent-id.md). |
