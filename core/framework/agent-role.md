...
The role standard defines the schema for role JSON files, the global key registry, and the canonical invocation form for any command that accepts a role argument. See [`core/collab/agent-model.md`](../collab/agent-model.md) for per-role join-time model recommendations and [`commands/collab/join/index.md`](../../commands/collab/join/index.md) for agentId capture semantics.
...
For the reference inventory of declared prohibitions across registered roles, see [`role-prohibitions.md`](../collab/role-prohibitions.md).
