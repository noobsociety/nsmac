...
| **P5** | Link hygiene | Links stay inside allowed command, role, core, and rules paths |
...
| **P11** | Self-containment | Required dependencies stay inside `commands/`, `core/`, `core/framework/`, or explicit QA harness targets |
...
- Namespace router links may target route playbooks in the same namespace, `../core/framework/*.md`, fragment-only anchors, and optional `https://` / `http://` context links.
- Route playbook links may target sibling route playbooks, `../../../core/framework/*.md`, `../../../core/**/*.md`, fragment-only anchors, and optional `https://` / `http://` context links.
- Required authority for command behavior may come only from command files, shared role JSON under `core/collab/roles/`, core contracts under `core/framework/`, or a `core/<ns>/_*.md` shared-dependency file within the same namespace.
