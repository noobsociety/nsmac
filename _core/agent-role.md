...
The role standard defines the schema for role JSON files, the global key registry, and the canonical invocation form for any command that accepts a role argument. See [`cursor/_functions/collab/_agent-model.md`](../_functions/collab/_agent-model.md) for per-role join-time model recommendations and [`cursor/_functions/collab/join.md`](../_functions/collab/join.md) for agentId capture semantics.
