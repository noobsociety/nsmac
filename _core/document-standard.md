...
Install layout and runtime symlink behavior are out of scope for the `_core/` canon.
...
Full playbook authoring for files under the config tree’s **`commands/`** and **`_functions/`** directories — catalog sync, heading order, link allowlists, harness exceptions, no manual TOC — lives in [command standard](command-standard.md).

**Opening sentence:** The first sentence is a single declarative statement that opens with the action verb. Exception: functions that define a named evaluation principle may open with a bold acronym label as the subject; mirror the opening line shape in `_functions/quality/assess-game.md` and reserve it for `/quality assess` functions only. Do not use the acronym-first opening for ordinary slash functions such as `_functions/git/commit.md`.
